# Performs preprocessing, identifies boundary using dev/PROT4

import cv2
import numpy as np
import math

# boundary buffer size
buffer = 50
...
def process_image(img, path):
    ...
    # save images
    cv2.imwrite(f"{path}/1_mask.JPEG", img_mask)
    cv2.imwrite(f"{path}/2_marked.JPEG", img_marked)
    cv2.imwrite(f"{path}/3_crop.JPEG", img_crop)
...
def calc_depth(left, right, path):
    # SIFT feature detect
    ...
    baseline = 30.48
    focal_len = 2714.3
    # focal length = 3.04mm. pixel size = 1.12um

    # calculate depth
    if weighted_avg > 1: depth = (focal_len * baseline) / weighted_avg 
    else: return 999999 # disparity too small

    # save image
    img_matches = cv2.drawMatches(left, kp1, right, kp2, matches, None, flags=2)
    cv2.imwrite(f"{path}/cv/match.JPEG", img_matches)
...
def process_imgs(dir):
    ...
    return calc_depth(left_crop, right_crop, dir)